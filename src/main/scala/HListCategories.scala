...
/**
  * Created by ryantanner on 5/25/16.
  */
object HListCategories {
  ...
  case class HoleCards[L <: HList : *->*[Card] : Constraint](cards: L) {
    def constraint = implicitly[Constraint[L]]
  }

  val shouldBePair = HoleCards(AceOfSpades :: AceOfHearts :: HNil).constraint
  val shouldnotwork = HoleCards(AceOfSpades :: TenOfHearts :: HNil).constraint

  //type HoleCards = Sized[Card, _2]

  trait Constraint[L <: HList]

  class Pair[C1 <: Card, C2 <: Card] private() extends Constraint[C1 :: C2 :: HNil]

  object Pair {
    implicit def pair[C1 <: Card, C2 <: Card](implicit ev: C1#S =:!= C2#S, ev1: C1#R =:= C2#R) = new Pair[C1, C2]
  }
